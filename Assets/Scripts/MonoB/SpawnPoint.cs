...
namespace BB
{
    ...
    public class SpawnPoint : MonoBehaviour
    {
        ...
        private void SpawnNextEnemy()
        {
            ...
            if (nextEnemy.Item1 == EnemyData.EnemyType.None) Debug.LogError("Failed to calculate enemy to spawn!");
        ...
        {
            var random = Random.Range(0, _spawnWeightTotal);
            float cumulative = 0;
            foreach (var kvp in _spawnData)
            {
                cumulative += kvp.Value.SpawnRate;
                if (random <= cumulative) return (kvp.Key, kvp.Value);
            }
